...
<p>The OpenAPI or Swagger component that is returned will have one additional property, <code>getBundledDefinition()</code>, that can be called to get back a JSON object that has the <code>$refs</code> resolved to only use a single file. This object can be converted to a string using <code>JSON.stringify()</code> to get the bundled definition. </p>
